...
static wymsg_chat_msg *WebYMsgMsgSent(char *file, size_t size)
{
    ...
    jm_txt = json_object_object_get(json_msg, "message");
    if (jm_txt == NULL) {
        json_object_put(json_msg);
        return NULL;
    }
    jm_from = json_object_object_get(json_msg, "sendAs");
    if (jm_from == NULL) {
        json_object_put(json_msg);
        return NULL;
    }
...
static wymsg_chat_msg *WebYMsgMsg(char *file, size_t size)
{
    ...
    int msg_len;
    wymsg_chat_msg *ret;
    struct json_object *json_msg, *jresp, *jarr, *jmsg, *jm_txt, *jm_to, *jm_from;
    ...
    /* new msg ?*/
    json_msg = json_tokener_parse(msg_raw);
    jresp = json_object_object_get(json_msg, "responses");
    if (jresp == NULL) {
        json_object_put(json_msg);
        return NULL;
    }
    ...
    jmsg = json_object_object_get(jarr, "message");
    jm_txt = json_object_object_get(jmsg, "msg");
    if (jm_txt == NULL) {
        json_object_put(json_msg);
        return NULL;
    }
    ...
    /* to */
    jm_to = json_object_object_get(jmsg, "receiver");
    to = json_object_to_json_string(jm_to);
    LogPrintf(LV_DEBUG, "Rt: %s", to);
    
    /* from */
    jm_from = json_object_object_get(jmsg, "sender");
    from = json_object_to_json_string(jm_from);
    LogPrintf(LV_DEBUG, "Rf: %s", from);
    
    /* message */
    msg = json_object_to_json_string(jm_txt);
    LogPrintf(LV_DEBUG, "%s %s", json_object_to_json_string(json_object_object_get(jmsg, "timeStamp")), msg);
...
static pei *WebYMsgIns(wymsg_chat_msg *msg, const pei *mpei)
{
    ...
    if (i == wymsg_dim) {
        ...
        sprintf(wymsg_list[ins].file, "%s/webymsg_chat_%p_%p_%li.txt", ProtTmpDir(), wymsg_list[ins].user, wymsg_list[ins].friend, time(NULL));
    ...
    for (i=0; i!=wymsg_dim; i++) {
        if (wymsg_list[i].user != NULL) {
            ...
            if (wymsg_list[i].ppei != NULL) {
                /* duration */
                sprintf(duration, "%lu", wymsg_list[i].last - wymsg_list[i].first);
