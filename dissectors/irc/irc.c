...
static irc_chat *IrcNewChannel(char *name)
{
    ...
    if (channel != NULL) {
        ...
        sprintf(channel->chat_msg, "%s/%s/channel_%s_%lu_%i_%p.txt", ProtTmpDir(), IRC_TMP_DIR, name, t, ++incr_channel, channel);
        ...
        sprintf(channel->users, "%s/%s/channel_%s_users_%lu_%i_%p.txt", ProtTmpDir(), IRC_TMP_DIR, name, t, incr_channel, channel);
        ...
        sprintf(channel->nick, "%s/%s/channel_%s_nick_%lu_%i_%p.txt", ProtTmpDir(), IRC_TMP_DIR, name, t, incr_channel, channel);
...
static int IrcConnec(int flow_id, irc_priv *priv)
{
    ...
    sprintf(irc.file_cmd, "%s/%s/irc_%lu_%p_%i.txt", ProtTmpDir(), IRC_TMP_DIR, time(NULL), &irc, incr);
    incr++;
