...
static http_resp HttpResp(const char *line, int len)
{
    return HTTP_RESP_NONE;
}
    ...
    do {
        ...
        if (end != NULL) {
            ...
            if (req == TRUE) {
                ...
                sprintf(msg->req_hdr_file, "%s/%s/http_rq_hdr_%lu_%p_%i.txt", ProtTmpDir(), HTTP_TMP_DIR,
                        time(NULL), msg, incr);
                incr++;
                ...
                sprintf(msg->res_hdr_file, "%s/%s/http_rs_hdr_%lu_%p_%i.txt", ProtTmpDir(), HTTP_TMP_DIR,
                        time(NULL), msg, incr);
            ...
            if (body) {
                ...
                if (req == TRUE) {
                    sprintf(msg->req_body_file, "%s/%s/http_rq_body_%lu_%p_%i", ProtTmpDir(), HTTP_TMP_DIR,
                            time(NULL), msg, incr);
                    ...
                    if (ext != NULL) {
                        sprintf(msg->res_body_file, "%s/%s/http_rs_body_%lu_%p_%i.%s", ProtTmpDir(), HTTP_TMP_DIR,
                                time(NULL), msg, incr, ext);
                    }
                    else {
                        sprintf(msg->res_body_file, "%s/%s/http_rs_body_%lu_%p_%i", ProtTmpDir(), HTTP_TMP_DIR,
                                time(NULL), msg, incr);
                    }
