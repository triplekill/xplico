...
static int TftpConnec(int flow_id, tftp_priv *priv)
{
    ...
    if (pkt != NULL) { /* useless */
        ...
        if (msg.oc == TFTP_OC_RRQ || msg.oc == TFTP_OC_WRQ) {
            ...
            sprintf(tftp.file_cmd, "%s/%s/tftp_%lu_%p_%i.txt", ProtTmpDir(), TFTP_TMP_DIR, time(NULL), &tftp, incr);
            incr++;
                ...
                if (pkt != NULL) {
                    ...
                    if (msg.oc == TFTP_OC_RRQ || msg.oc == TFTP_OC_WRQ) {
                        ...
                        if (mdata != NULL) {
                            ...
                            /* file path and name */
                            sprintf(new_data->file, "%s/%s/tftp_data_%lu_%p_%i_%i.txt", ProtTmpDir(), TFTP_TMP_DIR, time(NULL), &tftp, incr, i);
                            /* open file */
...
#warning "to do"
                ...
                if (data_id != -1) {
                    ...
                    new_data->gstack = ProtCopyFrame(FlowGrpStack(gid), TRUE);
                    /* file path and name */
                    sprintf(new_data->file, "%s/%s/tftp_data_%lu_%p_%i_%i_.txt", ProtTmpDir(), TFTP_TMP_DIR, time(NULL), &tftp, incr, i);
                    /* open file */
                    new_data->fp = fopen(new_data->file, "w");
