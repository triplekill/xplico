...
static int FtpConnec(int flow_id, ftp_priv *priv)
{
    ...
    sprintf(ftp.file_cmd, "%s/%s/ftp_%lu_%p_%i.txt", ProtTmpDir(), FTP_TMP_DIR, time(NULL), &ftp, incr);
    incr++;
    /* open file */
            ...
            if (ftp_dt != NULL) {
                if (ftp_dt->cmd == FTP_CMD_NONE) {
                    ...
                    if (ftp_dt->fp == NULL) {
                        ...
                        sprintf(ftp_dt->file, "%s/%s/ftp_%lu_%p_%i.bin", ProtTmpDir(), FTP_TMP_DIR, time(NULL), ftp_dt, incr);
                        incr++;
                        /* open file */
                            ...
                            if (ftpd->cmd == FTP_CMD_NONE) {
                                if (ftpd->fp == NULL) {
                                    ...
                                    if (ftpd->dim > FTP_DATA_BUFFER*100) {
                                        ...
                                        sprintf(ftpd->file, "%s/%s/ftp_%lu_%p_%i.txt", ProtTmpDir(), FTP_TMP_DIR, time(NULL), ftpd, incr);
                                        incr++;
                                        /* open file */
