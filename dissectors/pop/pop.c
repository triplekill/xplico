            ...
            if (msg->st != POP_ST_NONE) {
                if (msg->st == POP_ST_ERR)
                    ...
                    case POP_CMD_RETR:
                        ...
                        msg->data[dim] = '\0';
                        /* file path and name */
                        sprintf(msg->file_eml, "%s/%s/pop_%lu_%p_%i.eml", ProtTmpDir(), POP_TMP_DIR, time(NULL), msg, incr);
                        incr++;
                        /* open */
...
static int PopEmail(int flow_id, pop_con *priv)
{
    ...
    while (srv_msg != NULL) {
        ...
        if (srv_msg->cmdt == POP_CMD_RETR && srv_msg->file_eml[0] == '\0') {
            /* file name */
            sprintf(srv_msg->file_eml, "%s/%s/pop_%lu_%p_%i.eml", ProtTmpDir(), POP_TMP_DIR, time(NULL), srv_msg, incr);
            incr++;
        }
        PopPei(ppei, srv_msg);
        if (srv_msg->cmdt == POP_CMD_RETR) {
