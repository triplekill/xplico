...
static int PltkMsgRoom(unsigned long gid, const char *name, pltk_con *pltk, time_t start, unsigned long serial)
{
    ...
    if (room == NULL) {
        ...
        sprintf(room->chat_msg, "%s/%s/room_%s_%lu_%i_%p.txt", ProtTmpDir(), PLTK_TMP_DIR, name, t, ++incr_room, name);
        ...
        sprintf(room->users, "%s/%s/users_%s_%lu_%i_%p.txt", ProtTmpDir(), PLTK_TMP_DIR, name, t, ++incr_room, name);
        ...
        sprintf(room->nick, "%s/%s/nick_%s_%lu_%i_%p.txt", ProtTmpDir(), PLTK_TMP_DIR, name, t, ++incr_room, name);
...
static int PltkImMsg(pltk_im_msg *msg_in, pltk_con *pltk, time_t start, unsigned long serial, bool in)
{
    ...
    if (private == NULL) {
        ...
        sprintf(private->priv_msg, "%s/%s/private_%s_%lu_%i_%p.txt", ProtTmpDir(), PLTK_TMP_DIR, nick, t, ++incr_room, nick);
        ...
        sprintf(private->users, "%s/%s/users_%s_%lu_%i_%p.txt", ProtTmpDir(), PLTK_TMP_DIR, nick, t, ++incr_room, nick);
...
static void PltkConFree(pltk_con *pltk)
{
    ...
    for (private=pltk->private; private!=NULL; private=private->nxt) {
        ...
        PeiCompCapEndTime(cmp, private->end_time);
        sprintf(dur, "%lu", private->end_time-private->start_time);
    ...
    for (chat=pltk->chat; chat!=NULL; chat=chat->nxt) {
        ...
        PeiCompCapEndTime(cmp, chat->end_time);
        sprintf(dur, "%lu", chat->end_time-chat->start_time);
        PeiCompAddStingBuff(cmp, dur);
...
static bool PltkVerify(int flow_id)
{
    return PltkVerifyCheck(flow_id, FALSE);
}
