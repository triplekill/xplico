...
static inline int DispDirWebMsn(char *src, char *channel)
{
    char dir[XCLI_STR_DIM];
...
static int DispHttp(pei *ppei)
{
    ...
    if (path_file != NULL) {
        ...
        if (file_name == NULL) {
            ...
            sprintf(new_path, "%s/%s/httpfile/%s/%lu_%lu_%s", xdecode, ip_src, ip_dest, time(NULL), cnt_fl, file_name);
            cnt_fl++;
        }
...
static int DispDns(pei *ppei)
{
    ...
    if (ip_one != NULL || cname != NULL) {
        ...
        if (DispIp(ppei->stack, ip_str) == NULL)
            return -1;
        DispDirDns(ip_str);
        sprintf(new_path, "%s/%s/dns/dns_%lu.txt", xdecode, ip_str, tstart);
...
static int DispHttpFile(pei *ppei)
{
    ...
    if (file != NULL) {
        ...
        if (name == NULL) {
            ...
            snprintf(new_path, XCLI_STR_DIM, "%s/%s/httpfile/%s/%lu_%lu_%s", xdecode, ip_str, ip_dest, time(NULL), cnt_fl, name);
            cnt_fl++;
        }
...
static int DispArp(pei *ppei)
{
    ...
    if (mac != NULL && ip != NULL) {
        /* dir name and creation */
        DispDirArp();
        sprintf(new_path, "%s/arp/arp_%lu.txt", xdecode, tstart);
        arp_fp = fopen(new_path, "a");
        if (arp_fp != NULL) {
            fprintf(arp_fp, "%lu, %s -> %s\n", ppei->time_cap, mac, ip);
            fclose(arp_fp);
            DispFilePaths(new_path);
        }
    }
...
static int DispMsn(pei *ppei)
{
    ...
    if (ppei->ret == FALSE) {
        if (channel != NULL) {
            ...
            name = strrchr(cfile, '/');
            name++;
            sprintf(new_path, "%s/%s/msn/%s/%s_%lu", xdecode, ip_src, from, to, time(NULL));
            rename(cfile, new_path);
            DispFilePaths(new_path);
        }
    }
...
int DispInit(const char *cfg_file)
{
    ...
    sprintf(kml_file, "%s/geomap_%lu.kml", xdecode, (tstart/100)*100);
