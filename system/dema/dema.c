    ...
    while (1) {
            ...
            if (pd_tbl[i].run == FALSE) {
                ...
                if (filename != NULL) {
                    ...
                    if (SeDeStart(db_c, root, pd_tbl[i].pol_id, pd_tbl[i].sol_id, &pd_tbl[i].pid, FALSE, NULL, NULL) == 0) {
                        ...
                        printf("Applications executions error\n");
                        exit(-1);
                    }
                }
            }
                ...
                if (time_now && stat(path_src, &info) == 0) {
                    if (pd_tbl[i].size == info.st_size && SeDeFileActive(path_src) == FALSE) {
                        ...
                        if (ctime != NULL) {
                            ...
                            printf("Error: incorrect capture file %s\n", filename);
                            /* free */
                            pd_tbl[i].name[0] = '\0';
                            pd_tbl[i].size = 0;
                            pd_tbl[i].growth = 0;
        ...
        for (i=0, j=0; j<num; i++) {
            ...
            if (pd_tbl[i].run == FALSE && pd_tbl[i].size == 0) {
                ...
                if (nxt_ses != -1) {
                    printf("Created new session with id:%i\n", nxt_ses);
                    ...
                    if (pd_tbl[i].run == TRUE) {
                        if (SeDeRun(&pd_tbl[i].pid, chld, TRUE) == 0) {
                            ...
                            if (pd_tbl[i].end && filename == NULL) {
                                ...
                                SeDeKill(pd_tbl, i);
                                printf("Xplico or a Manipulator is dead!\n");
                            }
...
static int ReadConfigFile(char *path, dbconf *db_c, char *root_dir, time_t *twpcap, char *cert)
{
    ...
    while (fgets(buffer, CFG_LINE_MAX_SIZE, fp) != NULL) {
        ...
        if (strlen(buffer)+1 == CFG_LINE_MAX_SIZE) {
            printf("Config file line more length to %d characters", CFG_LINE_MAX_SIZE);
            return -1;
        }
        ...
        if (!CfgParIsComment(buffer)) {
            ...
            if (param != NULL) {
                if (dbts[0] != '\0') {
                    printf("Config param error: param '%s' defined two times", CFG_PAR_DB_TYPE);
                    return -1;
                }
            ...
            if (param != NULL) {
                if (db_c->file[0] != '\0') {
                    printf("Config param error: param '%s' defined two times", CFG_PAR_DB_FILE_NAME);
                    return -1;
                }
    ...
    /* check data */
    if (dbts[0] == '\0') {
        printf("Config file without DB type [%s]\n", CFG_PAR_DB_TYPE);
        return -1;
    }
    ...
    case DB_SQLITE:
        if (db_c->file[0] == '\0') {
            printf("Config file error. DB SQLite requires: %s\n", CFG_PAR_DB_FILE_NAME);
            return -1;
        }        
        break;
